...
image_file = tk.PhotoImage(file='c:/study/pic1.gif')

image = canvas.create_image(200, 0, anchor='n', image=image_file)
canvas.pack(side='top')
tk.Label(window, text='欢迎登录',font=('Arial', 16)).pack()

# 第5步，用户信息
tk.Label(window, text='用户名:', font=('Arial', 14)).place(x=10, y=170)
tk.Label(window, text='密    码:', font=('Arial', 14)).place(x=10, y=210)

# 第6步，用户登录输入框entry
# 用户名
var_usr_name = tk.StringVar()
var_usr_name.set('sunlignt')
...
def usr_login():
    ...
    try:
        with open('usrs_info.pickle', 'rb') as usr_file:  #usrs_info.pickle  前面可以随便写,后面不可以
    ...
    if usr_name in usrs_info:
        if usr_pwd == usrs_info[usr_name]:
            tkinter.messagebox.showinfo(title='Welcome', message='好久不见? ' + usr_name)
        # 如果用户名匹配成功，而密码输入错误，则会弹出'Error, your password is wrong, try again.'
        else:
            tkinter.messagebox.showerror(message='Error, 密码错误,请重新输入.')
    else:  # 如果发现用户名不存在
        is_sign_up = tkinter.messagebox.askyesno('Welcome！ ', '该账号不存在,请注册账号')
        # 提示需不需要注册新用户
        if is_sign_up:
            usr_sign_up()
...
def usr_sign_up():
    def sign_to_Hongwei_Website():
        ...
        if np != npf:
            tkinter.messagebox.showerror('Error', '密码不一致!')

        # 如果用户名已经在我们的数据文件中，则提示Error, The user has already signed up!
        elif nn in exist_usr_info:
            tkinter.messagebox.showerror('Error', '该账号已经被注册!')
        ...
        # 最后如果输入无以上错误，则将注册输入的信息记录到文件当中，并提示注册成功Welcome！,You have successfully signed up!，然后销毁窗口。
            ...
            tkinter.messagebox.showinfo('Welcome', '注册成功,请登录!')
            # 然后销毁窗口。
    ...
    tk.Label(window_sign_up, text='用户名: ').place(x=10, y=10)  # 将`用户名:`放置在坐标（10,10）。
